...
/*   Updated: 2019/11/09 16:51:25 by rbalbous         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		*find_space(t_page **info, t_page **page, size_t size, int *count)
{
	...
	ft_memcpy(parser, (t_block){size, 0, NULL}, sizeof(t_block));
	return (parser->address = init_page(page, g_type_size));
}

void		init_page_info(t_page **info, t_page **page, size_t type_size)
{
	t_page		*new;
		...
		else
		{
			if ((*info)->prev == NULL)
			{
				while ((*info)->next != NULL)
					*info = (*info)->next;
				init_page_info(info, page, type_size);
				return (add_block(info, page, type_size, size));
			}
			else
			{
				*info = (*info)->prev;
...
void		*malloc_tiny_small(size_t size, size_t type_size, t_page **page)
{
	...
	if (!(g_malloc_pages.info))
		init_page_info(&g_malloc_pages.info, page, type_size);
